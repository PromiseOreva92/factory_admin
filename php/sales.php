...
$limit = isset($_GET['limit']) ? (int) $_GET['limit'] : 20;
  ...
  $foreignKeyDetails = [
    ...
    $pageData = $db->fetchPaginatedDataFk('sales', $page, $limit, $foreignKeyDetails);
...
header('Content-Type: application/json');
    ...
    $amount = $_POST['amount'];
    $db->insert('sales', [ 'product_id' => $product, 'quantity' => $quantity, 'amount' => $amount ]);
    ...
    $amount = $_PUT['amount'];
    $db->update('sales', ['quantity' => $quantity, 'amount' => $amount ], "id = $id");
